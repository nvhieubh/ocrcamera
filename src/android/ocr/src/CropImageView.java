...
//        calculateImageScaleBounds();
     ...
     * {@link CropImageView.WrapCropBoundsRunnable} which animates image.
    ...
    public void setImageToWrapCropBounds(boolean animate) {
        if (mBitmapLaidOut && !isImageWrapCropBounds()) {
            ...
            if (animate) {
                post(mWrapCropBoundsRunnable = new CropImageView.WrapCropBoundsRunnable(
    ...
    protected void processStyledAttributes(@NonNull TypedArray a, R r) {
        float targetAspectRatioX = Math.abs(a.getFloat(r.getId("styleable", "ucrop_UCropView_ucrop_aspect_ratio_x"), DEFAULT_ASPECT_RATIO));
        float targetAspectRatioY = Math.abs(a.getFloat(r.getId("styleable", "ucrop_UCropView_ucrop_aspect_ratio_y"), DEFAULT_ASPECT_RATIO));
     ...
     * or when certain conditions inside {@link CropImageView.WrapCropBoundsRunnable#run()} method are triggered.
     */
     ...
     * Given values are interpolated during the animation time.
     * Runnable can be terminated either vie {@link #cancelAllAnimations()} method
     * or when certain conditions inside {@link CropImageView.ZoomImageToPosition#run()} method are triggered.
     */
